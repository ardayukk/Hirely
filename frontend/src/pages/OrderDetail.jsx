...
export default function OrderDetail() {
    ...
    const handleSubmitDelivery = async () => {
        try {
            ...
            formData.append('message', deliveryNote);
            deliveryFiles.forEach(file => {
                formData.append('files', file);
            });

            await axiosInstance.post(`/api/orders/${orderId}/deliveries`, formData, {
            ...
            alert(err.response?.data?.detail || 'Failed to submit delivery');
        } finally {
    ...
    return (
        <Container sx={{ mt: 4 }}>
                    ...
                    <Paper sx={{ p: 3, mb: 3 }}>
                        ...
                        {order.delivery_date && (
                            <Box sx={{ mb: 2 }}>
                                ...
                                <Typography variant="body1" color={timeLeft === 'Overdue' ? 'error' : 'text.primary'} fontWeight={timeLeft === 'Overdue' ? 'bold' : 'normal'}>
                                    {new Date(order.delivery_date).toLocaleString()} ({timeLeft})
                                </Typography>
                            </Box>
                        )}
            ...
            <Dialog open={deliveryOpen} onClose={() => setDeliveryOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogActions>
                    <Button onClick={() => setDeliveryOpen(false)} disabled={submittingDelivery}>
                        Cancel
                    </Button>
                    <Button
